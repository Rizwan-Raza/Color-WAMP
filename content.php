    ...
    <div id="post-<?php the_ID(); ?>" <?php post_class("card my-4"); ?>>
        ...
        <?php if (has_post_thumbnail() and ((is_single() and get_theme_mod("color_wamp_featured_image_show", 1)) or (is_page() and get_theme_mod("color_wamp_featured_image_single_page_show", 1)))) {
            ...
            <div class="card-image">
                <?php the_post_thumbnail("theme-wamp-banner-image", get_theme_mod("color_wamp_featured_image_popup") ? array("class" => "materialboxed banner-img") : array("class" => "banner-img"));
            } else {
                ?>
                ...
                <div class="card-content"> <?php
                    ...
                    <time class="grey-text left mr-2"><a href="<?php echo esc_url(get_day_link(get_the_modified_time('Y'), get_the_modified_time('m'), get_the_modified_time('d'))) ?>" class="grey-text primary-text-hover tooltipped" data-tooltip="<?php echo esc_attr(get_the_modified_time("D, j M Y \a\\t h:i A")) ?>"><i class="material-icons left mr-1">&#xe923</i> <?php echo esc_html(human_time_diff(get_the_modified_time('U'), current_time('timestamp'))) . ' ago' ?></a></time>
                <?php } ?>
                <time class="grey-text left mr-2"><a href="<?php echo esc_url(get_day_link(get_the_time('Y'), get_the_time('m'), get_the_time('d'))) ?>" class="grey-text primary-text-hover tooltipped" data-tooltip="<?php echo esc_attr(get_the_time("D, j M Y \a\\t h:i A")) ?>"><i class="material-icons left mr-1">&#xe192</i> <?php echo esc_html(human_time_diff(get_the_time('U'), current_time('timestamp'))) . ' ago' ?></a></time>
                ...
                <span class="left mr-2"><a href="<?php echo esc_url(get_comments_link()) ?>" class="grey-text primary-text-hover"><i class="material-icons left mr-1">&#xe0bf</i> <?php comments_number() ?> </a></span>
                ...
                $tags_list = get_the_tag_list('<div class="tag-links my-2"><i class="material-icons">&#xe54e</i>', ', ', '</div>');
                if ($tags_list) {
