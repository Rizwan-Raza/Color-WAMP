<?php while (have_posts()) {
    ...
    <?php if (has_post_thumbnail()) {
        ...
        ?>
          ...
          <a href="<?php the_permalink()?>" class="grey-text text-darken-3 primary-text-hover"><span class="card-title"><h3><?php the_title()?></h3></span></a>
            <?php if (has_post_thumbnail()) {
        ?> </div>
            ...
            <time class="left mr-2"><a href="<?php echo esc_attr(get_day_link(get_the_time('Y'), get_the_time('m'), get_the_time('d')))?>" class="grey-text primary-text-hover tooltipped" data-tooltip="Posted on: <?php echo esc_attr( get_the_time("D, j M Y \a\\t h:i A"))?>"><i class="material-icons left mr-1">&#xe192</i> <?php echo esc_html(human_time_diff(get_the_time('U'), current_time('timestamp'))) . ' ago'?></a></time>
            
            <span class="left mr-2"><a href="<?php echo esc_attr(get_author_posts_url(get_the_author_meta('ID'))) ?>" data-tooltip="Posts by <?php echo esc_attr(ucfirst(get_the_author()))?>" class="grey-text primary-text-hover tooltipped"><i class="material-icons left mr-1">&#xe7fd</i> <?php echo esc_html(ucfirst(get_the_author()))?></a></span>
            
            <span class="left mr-2"><a href="<?php echo esc_attr(get_comments_link())?>" class="grey-text primary-text-hover"><i class="material-icons left mr-1">&#xe0bf</i> <?php esc_html(comments_number())?> </a></span>
